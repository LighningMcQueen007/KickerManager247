...
def enforce_team_limit(team, max_pro_verein, fixed_players=None):
    """Filtert das Team, sodass max. X Spieler pro Verein bleiben, Wunschspieler bleiben erhalten."""
    ...
    for verein, players in verein_counter.items():
        # Wunschspieler immer behalten
        fixed = [p for p in players if p in fixed_players]
        rest = [p for p in players if p not in fixed_players]

        rest.sort(key=lambda x: x["Punkte"], reverse=True)
        allowed_rest = rest[:max_pro_verein - len(fixed)]
        new_team.extend(fixed + allowed_rest)
...
def main(path, wunschspieler=None, ausgeschlossen=None, max_spieler_pro_verein=1):

    if wunschspieler is None:
        wunschspieler = set()
    ...
    # Spieler ausschließen → Punkte auf 0 setzen
    for p in players_all:
        pname = (p.get("Angezeigter Name") or "").strip()
        if pname in ausgeschlossen:
            p["Punkte"] = 0
    ...
    # Vereinslimit
    team = enforce_team_limit(team, max_spieler_pro_verein, fixed_players=fixed_players)
